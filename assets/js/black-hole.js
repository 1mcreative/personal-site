// Black Hole — /life/'s hero background, replacing Accretion Disc (see
// CLAUDE.md tooling log; accretion-disc.js is DEPRECATED, not deleted, same
// convention as every other superseded effect on this site).
//
// Ported from a pasted React/WebGPU reference (index.tsx, renderer.ts,
// pipeline.ts, and four .wgsl shader files, built on a small internal "vgpu"
// wrapper around raw WebGPU). Dropped, same treatment every reference on this
// site gets: React/TypeScript scaffolding, and vgpu's device/target/effect/
// frame-loop plumbing — this site has no bundler, so the four shaders can't
// be `import`ed as modules either; they're inlined below as plain strings.
// The vertex shader (a full-screen triangle, shared by every pass) and all
// host-side device/pipeline/bind-group/render-pass code is written directly
// against raw WebGPU, since nothing that thin already exists in this
// codebase. The raymarching/geodesic math in the black hole fragment shader
// itself is untouched from the reference.
//
// RECOLORED, not just ported — same rule and same treatment accretion-disc.js
// (the effect this replaces) already got: the reference's disk is copper/
// orange (a dark red-brown dimming out to a warm near-white crest). This
// site's new visual elements draw from the site's own palette, so the disk's
// thermal gradient runs dark navy -> Grind's #1d4ed8 -> an icy near-white-
// blue crest on the approaching side, with the receding (redshifted) side
// warmed toward the site's amber accent (#f59e0b, reused from the homepage
// globe/glitter, not a new hue) — see volumeSample()'s `recede` mix. The
// starfield's per-star color temperature (meant to vary a real sky from
// blue-hot to warm-cool stars) is narrowed to a blue-to-white range for the
// same reason — realistic star color variance is fine, but an orange third
// of the sky next to a blue disk is exactly the "doesn't blend in" problem
// this site's already been burned by once (see DESIGN.md's Beyond Horizon
// writeup); the amber accent stays scoped to the disk's own redshift, not
// spread across the whole scene.
//
// The reference's pointer handling isn't drag-to-orbit — `pointermove`
// updates the camera target on *any* mouse movement over the canvas
// (pointerdown/up only guard against a second simultaneous pointer stealing
// it), so this is a hover-follow parallax camera, the same interaction shape
// as this site's own spotlight-text/hero-blobs. Gated behind
// `matchMedia('(hover: hover)')` — same reasoning and same gate
// spotlight-text.js already uses: a touch drag over this canvas would
// otherwise fight the page's own vertical scroll, since `.life-hero` is just
// the top of a longer scrollable page here, not the whole viewport the
// reference assumed. Touch visitors get a fixed camera angle; the disk still
// animates on its own (time only drives the turbulence/orbital swirl in the
// shader, never the camera), so nothing goes fully static.
(function () {
  var canvas = document.querySelector(".life-hero-canvas");
  if (!canvas || !navigator.gpu) return;

  var DPR_CAP = 1.6; // lower than accretion-disc.js's 2 — a 256-step-per-pixel
  // raymarch costs far more per pixel than a point-sprite cloud, so this
  // leans toward the reference's own more conservative cap rather than this
  // site's usual one.
  var BLOOM_MAX_H = 360;
  var BLURS = [
    { direction: [1, 0], radius: 1 },
    { direction: [0, 1], radius: 1 },
    { direction: [1, 0], radius: 2.4 },
    { direction: [0, 1], radius: 2.4 },
  ];

  // Shared by every pass: a full-screen triangle from just a vertex index,
  // no vertex buffer needed. uv.y is flipped relative to clip-space y (0 at
  // the top, 1 at the bottom) to match the pointer handler's own screen-space
  // convention below.
  var FULLSCREEN_VERT =
    "struct VSOut { @builtin(position) pos: vec4f, @location(0) uv: vec2f }\n" +
    "@vertex fn vs_main(@builtin(vertex_index) i: u32) -> VSOut {\n" +
    "  var p = array<vec2f, 3>(vec2f(-1.0, -1.0), vec2f(3.0, -1.0), vec2f(-1.0, 3.0));\n" +
    "  var t = array<vec2f, 3>(vec2f(0.0, 1.0), vec2f(2.0, 1.0), vec2f(0.0, -1.0));\n" +
    "  var out: VSOut;\n" +
    "  out.pos = vec4f(p[i], 0.0, 1.0);\n" +
    "  out.uv = t[i];\n" +
    "  return out;\n" +
    "}\n";

  var BLACK_HOLE_FRAG = `
struct Params {
  resolution: vec2f,
  pointer: vec2f,
  time: f32,
}

@group(0) @binding(0) var<uniform> params: Params;

const PI: f32 = 3.14159265359;
const HORIZON: f32 = 1.0;
const ISCO: f32 = 3.0;
const DISK_OUTER: f32 = 9.5;

fn hash21(p: vec2f) -> f32 {
  var q = fract(p * vec2f(123.34, 456.21));
  q += vec2f(dot(q, q + vec2f(45.32)));
  return fract(q.x * q.y);
}

fn noise(p: vec2f) -> f32 {
  let i = floor(p);
  let f = fract(p);
  let u = f * f * (3.0 - 2.0 * f);
  return mix(mix(hash21(i), hash21(i + vec2f(1.0, 0.0)), u.x),
             mix(hash21(i + vec2f(0.0, 1.0)), hash21(i + vec2f(1.0, 1.0)), u.x), u.y);
}

fn fbm(p0: vec2f) -> f32 {
  var p = p0;
  var value = 0.0;
  var amplitude = 0.5;
  for (var i = 0; i < 4; i++) {
    value += amplitude * noise(p);
    p = mat2x2f(1.6, 1.2, -1.2, 1.6) * p;
    amplitude *= 0.5;
  }
  return value;
}

fn geodesicAcceleration(position: vec3f, velocity: vec3f) -> vec3f {
  let r2 = dot(position, position);
  let angularMomentum = cross(position, velocity);
  let h2 = dot(angularMomentum, angularMomentum);
  return -1.5 * h2 * position / (r2 * r2 * sqrt(r2));
}

fn starField(direction: vec3f) -> vec3f {
  let d = normalize(direction);
  let spherical = vec2f(atan2(d.z, d.x) / (2.0 * PI), asin(clamp(d.y, -1.0, 1.0)) / PI);
  let grid = spherical * vec2f(720.0, 360.0);
  let cell = floor(grid);
  let local = fract(grid) - 0.5;
  let seed = hash21(cell);
  let radius = mix(0.02, 0.07, seed * seed);
  let point = smoothstep(radius, 0.0, length(local)) * step(0.986, seed);
  let glow = smoothstep(radius * 4.0, 0.0, length(local)) * step(0.997, seed) * 0.35;
  let temperature = hash21(cell + vec2f(17.0, 29.0));
  // Blue-to-white only (reference goes blue-to-orange) — see file header.
  let tint = mix(vec3f(0.55, 0.72, 1.0), vec3f(0.88, 0.94, 1.0), temperature);
  return tint * (point * (1.5 + seed * 3.0) + glow);
}

fn volumeSample(point: vec3f, rayVelocity: vec3f) -> vec4f {
  let radius = length(point.xz);
  let height = abs(point.y);
  if (radius <= ISCO || radius >= DISK_OUTER || height > 0.42) { return vec4f(0.0); }

  // Rotated Cartesian turbulence avoids a polar branch-cut seam.
  let omega = 0.42 / pow(radius, 1.5);
  let swirl = 2.2 * log(radius);
  let ang = params.time * omega + swirl;
  let c = cos(ang);
  let s = sin(ang);
  let rc = vec2f(c * point.x - s * point.z, s * point.x + c * point.z);
  let broad = fbm(rc * 0.9 + vec2f(0.0, params.time * 0.02));
  let detail = fbm(rc * 2.6 + broad * 1.5);
  let rings = 0.5 + 0.5 * sin(radius * 8.5 + broad * 6.0);
  let clumps = smoothstep(0.26, 0.84, broad * 0.72 + detail * 0.46 + rings * 0.22);

  let thickness = mix(0.05, 0.24, smoothstep(ISCO, DISK_OUTER, radius));
  let vertical = exp(-pow(height / thickness, 2.0) * 3.4);
  let innerFade = smoothstep(ISCO, ISCO + 0.45, radius);
  let outerFade = 1.0 - smoothstep(DISK_OUTER - 2.4, DISK_OUTER, radius);
  let radial = (DISK_OUTER - radius) / (DISK_OUTER - ISCO);
  let radialFalloff = pow(radial, 0.36);
  let density = vertical * innerFade * outerFade * radialFalloff * clumps;

  let heat = pow(radial, 1.35);
  // Site palette, not the reference's copper/orange — dark navy -> Grind
  // blue (#1d4ed8) -> an icy near-white-blue crest. See file header. The
  // receding (redshifted) side warms toward the site's own amber accent
  // (#f59e0b, the same amber used on the homepage's globe marker and
  // glitter field, not a new hue) instead of the crest's blue-white —
  // real black-hole renders show exactly this blue-approaching /
  // red-receding asymmetry, so it's a physically-motivated place to use
  // it as an accent rather than a wholesale recolor.
  var thermal = mix(vec3f(0.05, 0.11, 0.32), vec3f(0.114, 0.306, 0.847), smoothstep(0.05, 0.55, heat));

  let tangent = normalize(vec3f(-point.z, 0.0, point.x));
  let orbitalSpeed = min(0.64, 0.94 / sqrt(max(radius - HORIZON, 0.25)));
  let towardObserver = dot(tangent, -normalize(rayVelocity));
  // Gentle Doppler beaming keeps the threshold bloom nearly symmetric.
  let doppler = pow(clamp(1.0 / (1.0 - orbitalSpeed * towardObserver), 0.72, 1.55), 1.5);
  let recede = smoothstep(1.0, 0.62, doppler);
  thermal = mix(thermal, vec3f(0.961, 0.620, 0.043), recede * 0.6);
  thermal = mix(thermal, vec3f(0.85, 0.93, 1.0), pow(heat, 2.4));

  let gravitationalRedshift = sqrt(1.0 - HORIZON / radius);
  let emission = thermal * density * doppler * gravitationalRedshift * 9.5;
  return vec4f(emission, density * 2.1);
}

@fragment fn fs_main(@location(0) uv: vec2f) -> @location(0) vec4f {
  let aspect = params.resolution.x / params.resolution.y;
  let screen = (uv * 2.0 - 1.0) * vec2f(aspect, 1.0);

  let yaw = params.pointer.x;
  let pitch = clamp(params.pointer.y, -1.319, 1.319);
  let orbitRadius = 21.0;
  let cameraPosition = vec3f(
    sin(yaw) * cos(pitch) * orbitRadius,
    sin(pitch) * orbitRadius,
    cos(yaw) * cos(pitch) * orbitRadius,
  );
  let forward = normalize(-cameraPosition);
  let right = normalize(cross(forward, vec3f(0.0, 1.0, 0.0)));
  let up = cross(right, forward);

  var position = cameraPosition;
  var velocity = normalize(forward * 1.72 + right * screen.x + up * screen.y);
  var accumulated = vec3f(0.0);
  var transmittance = 1.0;
  var escaped = false;

  for (var stepIndex = 0; stepIndex < 256; stepIndex++) {
    let radius = length(position);
    if (radius < HORIZON * 1.015) { break; }
    if (radius > 24.0 && stepIndex > 24 && dot(position, velocity) > 0.0) {
      escaped = true; break;
    }

    // Step finer near the horizon where the geodesic curves hardest.
    var stepSize = clamp((radius - HORIZON) * 0.07, 0.016, 0.24);

    // Cap steps near the thin disk so rays cannot skip its slab.
    let rxz = length(position.xz);
    if (rxz > ISCO - 0.6 && rxz < DISK_OUTER + 0.6) {
      let slab = mix(0.05, 0.24, smoothstep(ISCO, DISK_OUTER, rxz));
      let vy = max(abs(velocity.y), 0.001);
      let band = slab * 3.0;
      let ay = abs(position.y);
      if (ay < band) {
        stepSize = min(stepSize, (slab * 0.4) / vy);
      } else if (position.y * velocity.y < 0.0) {
        stepSize = min(stepSize, (ay - band) / vy);
      }
      stepSize = max(stepSize, 0.004);
    }

    let previousPosition = position;

    let acceleration0 = geodesicAcceleration(position, velocity);
    velocity += acceleration0 * (0.5 * stepSize);
    position += velocity * stepSize;
    let acceleration1 = geodesicAcceleration(position, velocity);
    velocity += acceleration1 * (0.5 * stepSize);
    velocity = normalize(velocity);

    let samplePoint = mix(previousPosition, position, 0.5);
    let volume = volumeSample(samplePoint, velocity);
    if (volume.a > 0.0001 && transmittance > 0.008) {
      let opticalDepth = volume.a * stepSize;
      let absorbed = 1.0 - exp(-opticalDepth);
      accumulated += volume.rgb * transmittance * absorbed / max(volume.a, 0.001);
      transmittance *= exp(-opticalDepth);
    }
  }

  if (escaped) { accumulated += starField(velocity) * transmittance; }

  // Linear HDR output; tone mapping and bloom happen in the post pipeline.
  return vec4f(accumulated, 1.0);
}
`;

  var BRIGHT_PASS_FRAG = `
@group(0) @binding(0) var src: texture_2d<f32>;
@group(0) @binding(1) var samp: sampler;

@fragment fn fs_main(@location(0) uv: vec2f) -> @location(0) vec4f {
  let color = textureSampleLevel(src, samp, uv, 0.0).rgb;
  let luminance = dot(color, vec3f(0.2126, 0.7152, 0.0722));
  // Quadratic soft knee around the threshold for smooth highlight roll-in.
  let knee = 0.6;
  let soft = clamp((luminance - 1.0 + knee) / (2.0 * knee), 0.0, 1.0);
  let contribution = max(soft * soft * knee, luminance - 1.0);
  let weight = contribution / max(luminance, 0.0001);
  return vec4f(color * weight, 1.0);
}
`;

  var BLUR_FRAG = `
struct Blur {
  texelSize: vec2f,
  direction: vec2f,
  radius: f32,
}

@group(0) @binding(0) var src: texture_2d<f32>;
@group(0) @binding(1) var samp: sampler;
@group(0) @binding(2) var<uniform> blur: Blur;

@fragment fn fs_main(@location(0) uv: vec2f) -> @location(0) vec4f {
  // 9-tap Gaussian weights.
  let weights = array<f32, 5>(0.227027, 0.1945946, 0.1216216, 0.054054, 0.016216);
  let step = blur.texelSize * blur.direction * blur.radius;
  var result = textureSampleLevel(src, samp, uv, 0.0).rgb * weights[0];
  for (var i = 1; i < 5; i++) {
    let offset = step * f32(i);
    result += textureSampleLevel(src, samp, uv + offset, 0.0).rgb * weights[i];
    result += textureSampleLevel(src, samp, uv - offset, 0.0).rgb * weights[i];
  }
  return vec4f(result, 1.0);
}
`;

  var COMPOSITE_FRAG = `
@group(0) @binding(0) var scene: texture_2d<f32>;
@group(0) @binding(1) var bloom: texture_2d<f32>;
@group(0) @binding(2) var samp: sampler;

fn aces(x: vec3f) -> vec3f {
  let a = 2.51;
  let b = 0.03;
  let c = 2.43;
  let d = 0.59;
  let e = 0.14;
  return clamp((x * (a * x + vec3f(b))) / (x * (c * x + vec3f(d)) + vec3f(e)), vec3f(0.0), vec3f(1.0));
}

@fragment fn fs_main(@location(0) uv: vec2f) -> @location(0) vec4f {
  let hdrScene = textureSampleLevel(scene, samp, uv, 0.0).rgb;
  let hdrBloom = textureSampleLevel(bloom, samp, uv, 0.0).rgb;
  var color = hdrScene + hdrBloom * 0.9;

  color *= 1.15;
  color = aces(color);

  // Subtle cinematic vignette.
  let centered = uv - vec2f(0.5);
  let vignette = 1.0 - smoothstep(0.55, 1.15, length(centered) * 1.6);
  color *= mix(0.72, 1.0, vignette);

  color = pow(color, vec3f(1.0 / 2.2));
  return vec4f(color, 1.0);
}
`;

  var device, context, canvasFormat, sampler;
  var scenePipeline, brightPipeline, blurPipeline, compositePipeline;
  var sceneParamsBuf, blurParamBufs;
  var sceneBindGroup, brightBindGroup, blurBindGroups, compositeBindGroup;
  var sceneTarget, bloomTargets;
  var disposed = false;
  var pointer = [0, 0.05];
  var targetPointer = [0, 0.05];
  var startTime = 0;
  var resizeFrame = 0;
  var pendingW = 0;
  var pendingH = 0;
  var hasPending = false;
  var lastDpr = window.devicePixelRatio || 1;

  function fail(error) {
    if (error) console.error("black-hole:", error);
    disposed = true;
  }

  function makeModule(fragSource) {
    return device.createShaderModule({ code: FULLSCREEN_VERT + fragSource });
  }

  function makePipeline(module, format) {
    return device.createRenderPipeline({
      layout: "auto",
      vertex: { module: module, entryPoint: "vs_main" },
      fragment: { module: module, entryPoint: "fs_main", targets: [{ format: format }] },
      primitive: { topology: "triangle-list" },
    });
  }

  function makeTarget(w, h) {
    var width = Math.max(1, w);
    var height = Math.max(1, h);
    var texture = device.createTexture({
      size: [width, height],
      format: "rgba16float",
      usage: GPUTextureUsage.RENDER_ATTACHMENT | GPUTextureUsage.TEXTURE_BINDING,
    });
    return { texture: texture, view: texture.createView(), width: width, height: height };
  }

  function destroyTarget(target) {
    if (target) target.texture.destroy();
  }

  function updateBindGroups() {
    brightBindGroup = device.createBindGroup({
      layout: brightPipeline.getBindGroupLayout(0),
      entries: [
        { binding: 0, resource: sceneTarget.view },
        { binding: 1, resource: sampler },
      ],
    });

    var texelSize = [1 / bloomTargets[0].width, 1 / bloomTargets[0].height];
    blurBindGroups = BLURS.map(function (blur, i) {
      var data = new Float32Array(8);
      data[0] = texelSize[0];
      data[1] = texelSize[1];
      data[2] = blur.direction[0];
      data[3] = blur.direction[1];
      data[4] = blur.radius;
      device.queue.writeBuffer(blurParamBufs[i], 0, data);
      return device.createBindGroup({
        layout: blurPipeline.getBindGroupLayout(0),
        entries: [
          { binding: 0, resource: bloomTargets[i % 2].view },
          { binding: 1, resource: sampler },
          { binding: 2, resource: { buffer: blurParamBufs[i] } },
        ],
      });
    });

    compositeBindGroup = device.createBindGroup({
      layout: compositePipeline.getBindGroupLayout(0),
      entries: [
        { binding: 0, resource: sceneTarget.view },
        { binding: 1, resource: bloomTargets[0].view },
        { binding: 2, resource: sampler },
      ],
    });

    device.queue.writeBuffer(sceneParamsBuf, 0, new Float32Array([sceneTarget.width, sceneTarget.height]));
  }

  function rebuildTargets(w, h) {
    var bloomH = Math.min(BLOOM_MAX_H, h);
    var bloomW = Math.max(1, Math.round((bloomH * w) / Math.max(h, 1)));

    var oldScene = sceneTarget;
    var oldBloomA = bloomTargets && bloomTargets[0];
    var oldBloomB = bloomTargets && bloomTargets[1];

    sceneTarget = makeTarget(w, h);
    bloomTargets = [makeTarget(bloomW, bloomH), makeTarget(bloomW, bloomH)];
    updateBindGroups();

    destroyTarget(oldScene);
    destroyTarget(oldBloomA);
    destroyTarget(oldBloomB);
  }

  function applyResize() {
    resizeFrame = 0;
    if (disposed || !hasPending) return;
    hasPending = false;
    rebuildTargets(pendingW, pendingH);
  }

  function scheduleResize(w, h) {
    pendingW = w;
    pendingH = h;
    hasPending = true;
    if (!resizeFrame) resizeFrame = requestAnimationFrame(applyResize);
  }

  function measure() {
    var rect = canvas.getBoundingClientRect();
    var dpr = Math.min(DPR_CAP, Math.max(1, window.devicePixelRatio || 1));
    var w = Math.max(1, Math.round(rect.width * dpr));
    var h = Math.max(1, Math.round(rect.height * dpr));
    if (canvas.width !== w) canvas.width = w;
    if (canvas.height !== h) canvas.height = h;
    scheduleResize(w, h);
  }

  function onWindowResize() {
    var dpr = window.devicePixelRatio || 1;
    if (dpr === lastDpr) return;
    lastDpr = dpr;
    measure();
  }

  function installOrbitInput() {
    // Real mouse/trackpad only — see file header for why this is gated
    // instead of using touch-action:none the way the reference does.
    if (!window.matchMedia || !window.matchMedia("(hover: hover)").matches) return;

    var activePointer;

    function toTarget(clientX, clientY) {
      var rect = canvas.getBoundingClientRect();
      var x = Math.max(0, Math.min(1, (clientX - rect.left) / Math.max(1, rect.width)));
      var y = Math.max(0, Math.min(1, (clientY - rect.top) / Math.max(1, rect.height)));
      targetPointer[0] = (0.5 - x) * Math.PI * 1.4;
      targetPointer[1] = Math.max(-Math.PI * 0.42, Math.min(Math.PI * 0.42, (y - 0.5) * Math.PI * 0.7));
    }

    function down(event) {
      if (!event.isPrimary || activePointer !== undefined) return;
      activePointer = event.pointerId;
      if (canvas.setPointerCapture) canvas.setPointerCapture(event.pointerId);
    }

    function move(event) {
      if (!event.isPrimary) return;
      if (activePointer !== undefined && event.pointerId !== activePointer) return;
      toTarget(event.clientX, event.clientY);
    }

    function up(event) {
      if (event.pointerId !== activePointer) return;
      if (canvas.hasPointerCapture && canvas.hasPointerCapture(event.pointerId)) {
        canvas.releasePointerCapture(event.pointerId);
      }
      activePointer = undefined;
    }

    canvas.addEventListener("pointerdown", down);
    canvas.addEventListener("pointermove", move);
    canvas.addEventListener("pointerup", up);
    canvas.addEventListener("pointercancel", up);
  }

  function runPass(encoder, view, pipeline, bindGroup) {
    var pass = encoder.beginRenderPass({
      colorAttachments: [{ view: view, clearValue: { r: 0, g: 0, b: 0, a: 1 }, loadOp: "clear", storeOp: "store" }],
    });
    pass.setPipeline(pipeline);
    pass.setBindGroup(0, bindGroup);
    pass.draw(3);
    pass.end();
  }

  function frame(now) {
    if (disposed) return;
    requestAnimationFrame(frame);
    if (!sceneTarget) return;

    pointer[0] += (targetPointer[0] - pointer[0]) * 0.12;
    pointer[1] += (targetPointer[1] - pointer[1]) * 0.12;
    var t = (now - startTime) / 1000;
    device.queue.writeBuffer(sceneParamsBuf, 8, new Float32Array([pointer[0], pointer[1], t]));

    var encoder = device.createCommandEncoder();
    runPass(encoder, sceneTarget.view, scenePipeline, sceneBindGroup);
    runPass(encoder, bloomTargets[0].view, brightPipeline, brightBindGroup);
    for (var i = 0; i < 4; i++) {
      runPass(encoder, bloomTargets[(i + 1) % 2].view, blurPipeline, blurBindGroups[i]);
    }
    runPass(encoder, context.getCurrentTexture().createView(), compositePipeline, compositeBindGroup);
    device.queue.submit([encoder.finish()]);
  }

  async function init() {
    var adapter = await navigator.gpu.requestAdapter();
    if (!adapter || disposed) return fail();
    device = await adapter.requestDevice();
    if (disposed) return;
    device.addEventListener("uncapturederror", function (event) {
      console.error("black-hole device error:", event.error.message);
    });

    context = canvas.getContext("webgpu");
    if (!context) return fail();
    canvasFormat = navigator.gpu.getPreferredCanvasFormat();
    context.configure({ device: device, format: canvasFormat, alphaMode: "opaque" });

    sampler = device.createSampler({ minFilter: "linear", magFilter: "linear" });

    scenePipeline = makePipeline(makeModule(BLACK_HOLE_FRAG), "rgba16float");
    brightPipeline = makePipeline(makeModule(BRIGHT_PASS_FRAG), "rgba16float");
    blurPipeline = makePipeline(makeModule(BLUR_FRAG), "rgba16float");
    compositePipeline = makePipeline(makeModule(COMPOSITE_FRAG), canvasFormat);

    sceneParamsBuf = device.createBuffer({ size: 32, usage: GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST });
    blurParamBufs = BLURS.map(function () {
      return device.createBuffer({ size: 32, usage: GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST });
    });
    sceneBindGroup = device.createBindGroup({
      layout: scenePipeline.getBindGroupLayout(0),
      entries: [{ binding: 0, resource: { buffer: sceneParamsBuf } }],
    });

    installOrbitInput();

    var ro = new ResizeObserver(measure);
    ro.observe(canvas);
    window.addEventListener("resize", onWindowResize);
    measure();

    startTime = performance.now();
    requestAnimationFrame(frame);
  }

  init().catch(fail);
})();
