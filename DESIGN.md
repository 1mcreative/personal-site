...
- **Two zero-footprint easter eggs, homepage only:** a styled `console.log` pointing curious devs at the GitHub source, and a Konami-code keyboard listener (`hero.js`) that navigates straight to `/life/`. Neither adds any DOM/visual element and neither is a required path to anything — pure bonus. The Konami listener runs regardless of `prefers-reduced-motion` (it's a keyboard shortcut, not an animation); the resulting navigation still gets the same reduced-motion-aware view-transition as every other link.
