...
- **`.keycap-btn`** — "the personal button." Flat pill — an earlier tilted isometric version was hard to read — that floats up a few pixels on hover behind a strong, multi-layer sky-blue glow (`#12181f` fill, `#6ec1ff` glow, from the personal theme's palette). Also `clamp()`-sized. Every `color-mix()` value has a plain-`rgba()` fallback declared just before it: `color-mix()` failing to parse invalidates the *whole* `border`/`box-shadow` list in one shot (not just that layer), which is exactly "renders as bare text, no effect at all" — the fallback keeps a real border and glow even if that happens. Used for: the homepage's secondary CTA and the header nav link into `/life/` (on the professional page).
