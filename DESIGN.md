...
- Secondary CTA ("or peek at Localhost") to `/life/` — an isometric "keycap" button: a small tilted 3D panel with a glowing underside, adapted in plain CSS (via `@property`-registered custom-property transitions, no JS) from a second React/Framer-Motion reference the user provided. Colors pull from the personal theme's own palette (dark fill, `#6ec1ff` sky-blue glow) rather than the reference's default purple, so the button previews the theme behind the link — deliberately smaller and quieter than the primary button so it stays secondary. Two distinct button languages (flat dashed pill vs. 3D floating keycap) is an intentional pairing, not inconsistency: each side's entry point is styled like the side it leads to.
