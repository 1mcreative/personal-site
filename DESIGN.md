...
- **`.offset-btn`** — "the professional button." A dashed-border pill that lifts up-left on hover/focus with a hard offset shadow, adapted in plain CSS (transform + box-shadow transitions, no JS/animation library) from a React/Framer-Motion reference the user provided. Exactly two colors: `#1d4ed8` blue fill (constant everywhere) plus one "edge" color for the dashed border and hover shadow that each theme sets to match its own contrast (`--offset-edge: #14171c` near-black by default for light pages, overridden to `#ffffff` in `.theme-personal`) — not a third fixed hue like the original coral shadow. Text stays white regardless of theme: it has to read against the button's own blue fill, not the page. Shape never changes — earlier drafts also tightened the border-radius on hover, which read as the control turning into a different shape rather than just lifting; only position and shadow animate now. Used for: the homepage's primary CTA, the résumé's Email/LinkedIn actions, and the header nav link into `/resume/` (on the personal page).
- **`.keycap-btn`** — "the personal button." Started as a tilted isometric 3D prism (see the removed `@property`/`preserve-3d` machinery in earlier commits); that read as hard to parse and its glow barely showed, so it's now a flat pill — no tilt — that floats up a few pixels on hover behind a deliberately strong, multi-layer colored glow (`#12181f` dark fill, `#6ec1ff` sky-blue glow, from the personal theme's own palette). Simpler markup too: a single `<a class="keycap-btn">`, no nested scene/wall/top spans. Used for: the homepage's secondary CTA and the header nav link into `/life/` (on the professional page).
