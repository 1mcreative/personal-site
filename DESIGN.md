...
- **Galaxy button ("Explore"):** an overt CTA into `/life/`, sitting in `.hero-inner` below the subhead — unlike everything else on this list, not disguised as scenery. Ported from a plain HTML/CSS snippet the user supplied (no JS/framework attached to it) into this site's stack: a dark pill that's inert at rest (four faint twinkling dots), and on hover/focus reveals a spinning conic-gradient rim (`.spark`, mostly covered by `.backdrop` so only a thin glowing edge shows) plus a ring of 20 stars orbiting the button. The 20 stars share one CSS rule via a `--i` custom property set inline per star (`style="--i: N"`, generated by a Liquid `for` loop in `index.md`) rather than 20 hand-written `nth-child` blocks. All spin/twinkle animations drop out under `prefers-reduced-motion`, leaving a plain static pill.
