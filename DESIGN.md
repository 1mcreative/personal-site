...
- **The Chaos is a page corner, not an orb or a button:** `.page-corner` (top-right of the hero) reads as this page's own corner lifting slightly, like paper in a breeze — a dark triangle (`.page-corner-back`) sits under a white one (`.page-corner-front`) that flutters gently via a CSS `@keyframes` animation and peels back further on hover/focus, hinting at the dark page underneath. Replaced an earlier floating-dot `.portal-orb` per explicit feedback that it didn't work on desktop. It's a real `<a href="/life/">` with `aria-label`, not a hidden easter egg — the shapes are purely decorative (`aria-hidden`) since there's no text to carry the accessible name otherwise. Clicking it navigates with the sitewide wipe transition; a fuller "the page tears away, particles drift left" effect was explicitly asked for but deferred as its own later phase — that needs canvas, not CSS, to do properly.
